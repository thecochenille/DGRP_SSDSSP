...
pupa_mean_F01$PF01<-pupa_mean_F01$pupaFmean_1-pupa_mean_F01$pupaFmean_0
head(pupa_mean_F01)
```
...
pupa_mean_M01$PM01<-pupa_mean_M01$pupaMmean_1-pupa_mean_M01$pupaMmean_0
head(pupa_mean_M01)
```
...
library(lattice)
library(Deducer)
pupa_SSP<-merge(x= pupa_mean_M01, y= pupa_mean_F01, by.x="line", by.y="line")
head(pupa_SSP)
...
```{r}
pairs(pupa_SSP[1:8], lower.panel=panel.smooth, upper.panel=panel.cor)
...
pairs(fec_femsize[2:10], lower.panel=panel.smooth, upper.panel=panel.cor)
