...
head(df1_mean) #157 lines left after calculating SSD1
```
...
head(df2_mean) #59 lines left after I calculate SSD2
...
```{r}
#combining SSDs values without discarding rows
SSD0<-df0_mean[,c(1,4)]
SSD0$day<-"D0"
...
DOES IT MAKE SENSE TO COMPARE CORRELATIONS (is there a statistical test), I want to know if female Day0 variation controls more SSD0 than Day1 female size variation does for SSD2
...
We have shown that SSD in different condition changes, that in starved conditions, we have a decrease in overall SSD, and that the variation of SSD partly covariates with female size. But the degree of correlation varies? (how to test that?)
...
head(pupa_mean_F012)
pupa_plasticityF<-pupa_mean_F012 #this dataframe has female size in three days and plasticity
```

```{r}
#calculate plasticity male
head(df1)
df1F<-subset(df1, sex=="M")
df1F_mean<-aggregate(df1F[, 8], list(df1F$line), mean)
colnames(df1F_mean)<-c("line","pupaMmean_1")
head(df1F_mean)


df0F<-subset(df0, sex=="M")
df0F_mean<-aggregate(df0F[, 8], list(df0F$line), mean)
colnames(df0F_mean)<-c("line","pupaMmean_0")
head(df0F_mean)

df2F<-subset(df2, sex=="M")
df2F_mean<-aggregate(df2F[, 8], list(df2F$line), mean)
colnames(df2F_mean)<-c("line","pupaMmean_2")
head(df2F_mean)
...
pupa_mean_F012$PM01<-pupa_mean_F012$pupaMmean_0-pupa_mean_F012$pupaMmean_1
pupa_mean_F012$PM02<-pupa_mean_F012$pupaMmean_0-pupa_mean_F012$pupaMmean_2
pupa_mean_F012$PM12<-pupa_mean_F012$pupaMmean_1-pupa_mean_F012$pupaMmean_2

head(pupa_mean_F012)
pupa_plasticityM<-pupa_mean_F012
head(pupa_plasticityM) #this dataframe has MALE size in three days and plasticity
...
pupa_SSP<-merge(x=pupa_plasticityF, y= pupa_plasticityM, by.x="line", by.y="line") #merging male and female datasets
head(pupa_SSP)
...
```{r}
SSP01<-pupa_summary[c(1,14,17,18)]
...
```{r}
head(pupa_summary)
SSP01size<-pupa_summary[,c(1,3,9,14)]
head(SSP01size)
chart.Correlation(SSP01size[2:4],
                   method="pearson",
                   histogram=TRUE,
                   pch=16)
