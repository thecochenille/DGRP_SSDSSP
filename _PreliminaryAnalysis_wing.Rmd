...
To answer these questions, I will use the size of fed flies and flies starved for 1 day before pupariation.
...
```{r,message=FALSE, warning=FALSE}
#how many NA in wing)
sum(is.na(df$wing)) #638 Nas
#remove NA in wing column
df_sub<-df[!is.na(df$wing), ] #20772

#removing all groups with less than 10 flies
df_sub<-df_sub%>%
group_by(group) %>%
filter(n() >=10)  #19516 rows

#there is an outlier, fo a histo
df_sub<- subset(df_sub,wing >= 11.5)

#subset fed and 1day starved flies
df_sub01<-subset(df_sub, day=="D0"|day=="D1") #16577 rows

#lm with block fixed effect
blocktest01<-lm(wing~block, data=df_sub01)
summary(blocktest01)
Anova(blocktest01)

#move residuals to dataframe
df_sub01$wing_noblock <- blocktest01$resid
head(df_sub01)
...
blocktest01_2<-lm(wing_noblock~block, data=df_sub01)
...
# SSD in fed flies
  - Question 1.1: Is there Wing Sexual Size Dimorphism in the DGRP lines. A1.1: YES
...
df0<-subset(df, day=="D0") #9098 rows
head(df0,2) 
...
SSDtest<-lmer(wing~sex+(1|line) +(1|block), REML=TRUE, data=df0)
...
```{r}
(SSDtest_plot <- ggplot(df0, aes(x = sex, y = wing, colour = sex)) +
      geom_boxplot(alpha=0.3) +
      theme_classic() +
   geom_jitter(color="black", size=0.4, alpha=0.2)+
      geom_line(data = cbind(df0, pred = predict(SSDtest)), aes(y = pred), size = 1) +  # adding predicted line from mixed model 
...
model2<-lmer(wing~sex+(1|line)+(1|block), data=df0)  #model to test for SSD presence as we did above
model1<-lmer(wing~sex+(sex|line)+(1|block), data=df0) 
anova(model1)
anova(model1,model2)
```
...
I need to calculate summary for pupa female size, wing male size and SSD.

```{r}
#calculate means for each group using wing_noblock
head(df0)
...
head(df0_mean)
df0_mean %>%
  ggplot(aes(sex,wing_noblock, fill=sex)) +
  geom_boxplot() +
...
chart.Correlation(df0_mean[2:4],
                   ...
                   pch=16)
```
It seems that SSD in fed flies co-varies with the most with female wing size. 
Can we see that by looking back at the SSD model and look at the variance
...
```{r}
# Testing effect of sex in pupa size, with random effect for line and block.
SSD1<-lmer(pupa~sex*day+(sex*day|line) +(1|block), REML=TRUE, data=df01)
summary(SSD1)
anova(SSD1)
```
